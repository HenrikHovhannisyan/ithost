...
(function ($, undefined) {
  ...
  $.ui.slider.prototype._refreshValue = function () {
    ...
    if (self.orientation === "horizontal") {
      ...
      paddedBarPercent =
        ((elementWidth - (o.paddingMin + o.paddingMax)) * 100) / elementWidth;
    } else {
...
$(document).ready(function () {
  ...
  $("#slider").slider({
    ...
    slide: function (event, ui) {
      ...
      // Update other values
      $(".slider-container #storage_val span.value").html(
        storage_arr[ui.value - 1]
      );
      $(".slider-container #processor_val span.value").html(
        processor_arr[ui.value - 1]
      );
      $(".slider-container #ram_val span.value").html(ram_arr[ui.value - 1]);
      $(".slider-container #bandwidth_val span.value").html(
        bandwidth_arr[ui.value - 1]
      );
      $(".slider-container a.buynow-button").attr(
        "href",
        b_url + link_arr[ui.value - 1]
      );

      $(".slider-container div.price_rangetxt div").removeClass("current");
      for (var i = 0; i < ui.value; i++) {
        $(".slider-container div.price_rangetxt div#icon-" + i).addClass(
          "current"
        );
      }
    },
